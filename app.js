/* ============================================================
   Relay — Creative Ops for Brick&Bolt marketing
   Requirement → Content → Creative → Review → Approved, with
   per-stage turnaround (TAT) and SLA breach flags.
   ============================================================ */
(() => {
'use strict';
...
const LS = { me: 'relay.me', boardMode: 'relay.boardMode', local: 'relay.local.v1' };
const H = 3600e3, D = 24 * H, MIN = 60e3;

const DEFAULT_ROLES = [
  { id: 'admin',     name: 'Admin',     color: '#1F4FD1', desc: 'Manages the team, the flow and settings' },
  { id: 'requester', name: 'Marketing', color: '#64748B', desc: 'Raises requirements, reviews and approves the final output' },
  { id: 'content',   name: 'Content',   color: '#1baf7a', desc: 'Writes copy, scripts and messaging' },
  { id: 'creative',  name: 'Creative',  color: '#4a3aa7', desc: 'Designs and produces the final files' },
];
const DEFAULT_FLOW = {
  version: 1,
  stages: [
    { id: 'intake',   name: 'Requirement', role: 'requester', slaHours: 0,  kind: 'start',  color: '#64748B' },
    { id: 'content',  name: 'Content',     role: 'content',   slaHours: 24, kind: 'work',   color: '#1baf7a', needsFile: false },
    { id: 'creative', name: 'Creative',    role: 'creative',  slaHours: 48, kind: 'work',   color: '#4a3aa7', needsFile: true },
    { id: 'review',   name: 'Review',      role: 'requester', slaHours: 24, kind: 'review', color: '#eb6834', editsTo: 'creative' },
    { id: 'done',     name: 'Approved',    role: '',          slaHours: 0,  kind: 'end',    color: '#0ca30c' },
  ],
};
const DEFAULT_SETTINGS = {
  prefix: 'BB',
  types: ['Social post', 'Ad creative', 'Blog / article', 'Video', 'Email', 'Landing page', 'Brochure / print', 'Presentation', 'Other'],
...
const slug = (s) => lower(s).replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, '') || 'stage';
...
function whoHtml(p, size = 'sm') {
  if (!p) return `<span class="who">${personHtml(null, size)}<span class="name muted">Unassigned</span></span>`;
...
function fmtHrs(ms) { const h = ms / H; return h >= 100 ? Math.round(h) + 'h' : (Math.round(h * 10) / 10) + 'h'; }
...
class LocalStore {
  ...
  static seed() {
    ...
    if (S) {
      S.demoMembers.forEach((m) => { data.members[lower(m.email)] = { ...m, addedAt: now() }; });
...
const S = {
  ...
  configLoaded: false, loaded: { members: false, requests: false, config: false },
  route: { view: 'dashboard', id: null }, lastView: 'dashboard',
  q: '', filters: { type: '', prio: '', mine: false, breached: false, stage: '' }, sort: { key: 'updatedAt', dir: 'desc' },
  boardMode: localStorage.getItem(LS.boardMode) || 'board',
  drawer: null, modal: null, gateError: '', bootstrap: false, dbError: null,
};
...
const firstWorkStage = () => stages().find((s) => s.kind === 'work') || stages()[1];
...
const stageSlaMs = (st) => (st && st.slaHours > 0 ? st.slaHours * H : 0);
const visitBreached = (v) => { const st = stageById(v.stage); const sla = stageSlaMs(st); return sla > 0 && visitElapsed(v) > sla; };
...
function slaState(r) {
  ...
  if (!st || !v || r.status !== 'open') return { ratio: null, state: 'none', elapsed: v ? visitElapsed(v) : 0, sla: 0 };
  const sla = stageSlaMs(st), elapsed = visitElapsed(v);
  if (!sla) return { ratio: null, state: 'none', elapsed, sla: 0 };
  const ratio = elapsed / sla;
  return { ratio, elapsed, sla, remaining: sla - elapsed, state: ratio >= 1 ? 'crit' : ratio >= (S.settings.slaWarnAt || 0.75) ? 'warn' : 'ok' };
}
const anyBreach = (r) => (r.visits || []).some(visitBreached);
...
function needsMe(r) {
  ...
  if (st.kind === 'review') return isRequester(r) ? 'review' : null;
  if (st.kind === 'work') {
    const a = (r.assignees || {})[st.id];
    if (a && isMe(a)) return 'work';
    if (!a && hasRole(S.me, st.role)) return 'claim';
  }
  return null;
}
/* ---------------- permissions ---------------- */
const canCreate = () => !!S.me;
const canAssign = (r, st) => isAdmin() || isRequester(r) || hasRole(S.me, st.role) || isMe((r.assignees || {})[st.id]);
const canComplete = (r, st) => { const a = (r.assignees || {})[st.id]; return isAdmin() || isMe(a) || (!a && hasRole(S.me, st.role)); };
const canReview = (r) => isAdmin() || isRequester(r);
const canEditRequest = (r) => isAdmin() || isRequester(r);
...
function enterStage(r, stageId) {
  ...
  if (st && st.kind === 'end') { r.status = 'done'; r.completedAt = t; } else { r.status = 'open'; delete r.completedAt; }
  r.updatedAt = t;
}
...
async function createRequest(f) {
  ...
  const first = firstWorkStage(); const start = stages()[0]; const t = now();
  const r = {
    id, num, title: f.title, brief: f.brief, type: f.type, priority: f.priority, dueDate: f.dueDate || null, refs: f.refs,
  ...
  await saveRequest(r, `${id} raised — now in ${first.name}`);
  return r;
}
async function actAssign(id, stageId, email) {
  const r = clone(getReq(id)); const st = stageById(stageId);
  const m = email ? activeMembers().find((x) => sameEmail(x.email, email)) : null;
  if (email && !m) return toast('That teammate is not on the roster.', 'crit');
  r.assignees ||= {}; if (m) r.assignees[stageId] = pick(m); else delete r.assignees[stageId];
  const v = currentVisit(r); if (v && v.stage === stageId) v.assignee = m ? pick(m) : null;
  r.thread.push(sysMsg(m ? `assigned ${st.name} to ${isMe(m) ? 'themselves' : m.name}` : `cleared the ${st.name} assignee`));
  await saveRequest(r, m ? `${st.name} → ${m.name}` : 'Assignee cleared');
}
async function actComplete(id) {
  ...
  if (st.needsFile && !(r.deliverables || []).some((d) => d.round === r.round)) return toast(`Add the final file link before marking ${st.name} done.`, 'crit');
  if (!(r.assignees || {})[st.id] && hasRole(S.me, st.role)) { r.assignees[st.id] = pick(S.me); const v = currentVisit(r); if (v) v.assignee = pick(S.me); }
  closeVisit(r, 'completed'); enterStage(r, nx.id);
  r.thread.push(sysMsg(`marked ${st.name} done → ${nx.name}`));
  await saveRequest(r, `${st.name} done — moved to ${nx.name}`);
}
async function actSendBack(id, note) {
  const r = clone(getReq(id)); const st = stageById(r.stage); const pv = prevStage(r.stage);
  const target = pv && pv.kind !== 'start' ? pv : null;
  if (!target) return toast('There is no earlier stage to send this back to.', 'crit');
  closeVisit(r, 'sent-back'); enterStage(r, target.id);
  r.thread.push({ at: now(), by: pick(S.me), kind: 'edit', text: note || `Sent back from ${st.name} to ${target.name}.` });
  await saveRequest(r, `Sent back to ${target.name}`);
}
async function actApprove(id, note) {
  const r = clone(getReq(id)); const st = stageById(r.stage); const nx = nextStage(r.stage);
  if (!nx) return toast('No stage after review — check the flow.', 'crit');
  closeVisit(r, 'approved'); enterStage(r, nx.id);
  r.thread.push({ at: now(), by: pick(S.me), kind: 'approval', text: note || 'Approved.' });
  await saveRequest(r, `${r.id} approved`);
}
async function actRequestEdits(id, note) {
  const r = clone(getReq(id)); const st = stageById(r.stage);
  const target = stageById(st.editsTo) || stages().slice(0, stageIdx(st.id)).reverse().find((s) => s.kind === 'work');
  ...
  await saveRequest(r, `Edits requested — back to ${target.name} (round ${r.round})`);
}
async function actReopen(id) {
  const r = clone(getReq(id));
  const review = stages().find((s) => s.kind === 'review');
  const target = (review && stageById(review.editsTo)) || firstWorkStage();
  r.round = (r.round || 1) + 1;
...
async function actEdit(id, f) {
  const r = clone(getReq(id));
  Object.assign(r, { title: f.title, brief: f.brief, type: f.type, priority: f.priority, dueDate: f.dueDate || null, refs: f.refs });
  r.thread.push(sysMsg('updated the requirement details'));
...
async function saveMember(m, isNew) {
  ...
  if (i >= 0) S.members[i] = { ...S.members[i], ...doc, id }; else S.members.push({ ...doc, id });
  renderAll(false);
  try { await S.store.set('members', id, doc); toast(isNew ? `${doc.name} added to the team` : 'Teammate updated'); }
...
async function removeSamples() {
  ...
  const mids = S.members.filter((m) => m.sample).map((m) => m.id);
  S.requests = S.requests.filter((r) => !r.sample); S.members = S.members.filter((m) => !m.sample); renderAll(false);
  try { for (const id of ids) await S.store.del('requests', id); for (const id of mids) await S.store.del('members', id); toast('Sample data removed'); }
  catch (e) { toast(friendlyError(e), 'crit'); }
}
...
function metrics() {
  ...
  S.requests.forEach((r) => (r.visits || []).forEach((v) => { const st = stageById(v.stage); if (v.exitedAt && v.exitedAt >= cutoff && stageSlaMs(st) > 0) { total++; if (!visitBreached(v)) within++; } }));
  ...
  const tat = stages().filter((s) => s.kind === 'work' || s.kind === 'review').map((s) => {
    const vs = []; S.requests.forEach((r) => (r.visits || []).forEach((v) => { if (v.stage === s.id && v.exitedAt) vs.push(visitElapsed(v)); }));
    return { stage: s, avg: vs.length ? vs.reduce((a, b) => a + b, 0) / vs.length : null, n: vs.length, sla: stageSlaMs(s) };
  });
  ...
  return {
    ...
    avgE2E, recentDoneN: recentDone.length, onTime: total ? within / total : null, onTimeN: total, wip, tat, weeks,
  };
}
function activityFeed(limit = 12) {
  ...
  S.requests.forEach((r) => {
    ...
    (r.visits || []).forEach((v) => { if (v.exitedAt && v.action && v.action !== 'submitted') { const st = stageById(v.stage); const map = { completed: [`marked <b>${esc(st ? st.name : v.stage)}</b> done on <b>${esc(r.id)}</b>`, 'check', ''], approved: [`approved <b>${esc(r.id)}</b> ${esc(r.title)}`, 'check', 'good'], edits: [`requested edits on <b>${esc(r.id)}</b> (round ${(v.round || 1) + 1})`, 'rotate', 'crit'], 'sent-back': [`sent <b>${esc(r.id)}</b> back`, 'arrowLeft', ''], cancelled: [`cancelled <b>${esc(r.id)}</b>`, 'x', ''], reopened: [`reopened <b>${esc(r.id)}</b>`, 'rotate', ''] }[v.action]; if (map) ev.push({ at: v.exitedAt, by: v.by, r, icon: map[1], cls: map[2], text: map[0] }); } });
...
/* ---------------- shared components ---------------- */
function stageChip(st) { return st ? `<span class="chip stage" style="--stage:${attr(st.color)}"><span class="dot"></span>${esc(st.name)}</span>` : ''; }
...
function slaLine(r, compact = false) {
  ...
  if (r.status !== 'open') return `<div class="sla-line"><span>${r.status === 'done' ? 'Approved' : 'Cancelled'} · total ${fmtDur(totalTat(r))}</span></div>`;
  if (s.state === 'none') return `<div class="meter none"></div><div class="sla-line"><span>${compact ? 'In stage' : 'In ' + esc(st ? st.name : '')}</span><span class="t">${fmtDur(s.elapsed)} · no SLA</span></div>`;
  const pct = clamp(s.ratio * 100, 0, 100);
  const label = s.state === 'crit' ? `<span class="crit">Breached by ${fmtDur(-s.remaining)}</span>` : s.state === 'warn' ? `<span class="warn">${fmtDur(s.remaining)} left</span>` : `<span>${fmtDur(s.remaining)} left</span>`;
  return `<div class="meter ${s.state === 'ok' ? '' : s.state}"><i style="width:${pct.toFixed(1)}%"></i></div><div class="sla-line"><span class="t">${fmtDur(s.elapsed)} / ${st.slaHours}h</span>${label}</div>`;
}
function cardHtml(r) {
  const st = stageById(r.stage); const a = personFor(r); const breached = isBreachedNow(r); const due = fmtDue(r.dueDate, r.status);
  return `<article class="card ${breached ? 'breached' : ''}" data-open="${attr(r.id)}" data-flip-id="${attr(r.id)}" style="--stage:${attr(st ? st.color : '#888')}" tabindex="0" role="button" aria-label="${attr(r.id + ' ' + r.title)}">
    <div class="c-top"><span class="c-id">${esc(r.id)}</span>${r.round > 1 ? `<span class="round" title="Revision round">R${r.round}</span>` : ''}${r.sample ? '<span class="chip tiny" title="Sample data">sample</span>' : ''}<span style="flex:1"></span>${breached ? `<span class="breach-flag pulse">${ic('alert')}SLA</span>` : ''}</div>
    ...
    ${slaLine(r, true)}
    <div class="c-foot">${whoHtml(a)}<span class="tiny muted" title="Raised by ${attr(r.requester ? r.requester.name : '')}">by ${esc(r.requester ? r.requester.name.split(' ')[0] : '?')}</span></div>
  </article>`;
}
function personFor(r) { const st = stageById(r.stage); if (!st) return null; if (st.kind === 'review') return r.requester || null; if (st.kind === 'end' || r.status !== 'open') return r.requester || null; return (r.assignees || {})[st.id] || null; }
function emptyHtml(icon, text) { return `<div class="empty">${ic(icon)}${esc(text)}</div>`; }

/* ---------------- charts (inline SVG) ---------------- */
function chartBarsH(rows, opts = {}) {
  const w = 420, rowH = 30, left = 84, right = 96, top = 4; const h = top + rows.length * rowH + 4;
...
function chartBullet(rows) {
  const max = Math.max(1, ...rows.map((r) => Math.max(r.avg || 0, r.sla || 0))) * 1.12;
  return `<div class="stack" style="gap:2px">${rows.map((r) => {
    const over = r.sla > 0 && r.avg != null && r.avg > r.sla;
    const fill = r.avg == null ? 0 : (r.avg / max) * 100; const tgt = r.sla ? (r.sla / max) * 100 : null;
    const tip = `${r.stage.name}: avg ${r.avg == null ? 'no data' : fmtDur(r.avg)}${r.sla ? ' · SLA ' + r.stage.slaHours + 'h' : ''} · ${plural(r.n, 'completed visit')}`;
    return `<div class="bullet-row" data-tip="${attr(tip)}" tabindex="0"><div class="b-label" style="--c:${attr(r.stage.color)}"><i></i>${esc(r.stage.name)}</div><div class="bullet-track"><div class="fill ${over ? 'over' : ''}" data-w="${fill.toFixed(1)}"></div>${tgt != null ? `<div class="tgt" style="left:${tgt.toFixed(1)}%" title="SLA ${r.stage.slaHours}h"></div>` : ''}</div><div class="b-val ${over ? 'over' : ''}"><b>${r.avg == null ? '—' : fmtDur(r.avg)}</b>${r.sla ? ` <span class="muted">/ ${r.stage.slaHours}h</span>` : ''}</div></div>`;
  }).join('')}</div>`;
}
function chartLines(weeks) {
  ...
  series.forEach((s) => {
    ...
    const last = weeks[weeks.length - 1][s.key];
    out += `<circle class="dot" cx="${x(weeks.length - 1)}" cy="${y(last)}" r="4" fill="${s.color}"/>`;
    out += `<text class="val" x="${x(weeks.length - 1) + 8}" y="${y(last) + 4}">${last} ${esc(s.name.toLowerCase())}</text>`;
  });
...
function viewDashboard() {
  const m = metrics(); const first = (S.me && S.me.name.split(' ')[0]) || 'there';
  ...
  return `<div class="page-head" data-anim><div><h1>${greet}, ${esc(first)}</h1><p class="lede">${m.mine ? `<b>${plural(m.mine, 'request')}</b> waiting on you` : 'Nothing is waiting on you'} · ${plural(m.open, 'open request')} across the relay${m.breached ? ` · <span style="color:var(--crit);font-weight:600">${plural(m.breached, 'SLA breach', 'SLA breaches')}</span>` : ''}</p></div>
  ...
  <div class="stack">
    <div class="grid kpis">
      ...
      ${kpi('SLA breached now', m.breached, m.overdue ? `${ic('calendar', 'sm')} ${plural(m.overdue, 'request')} past due date` : 'Open stages over their SLA', m.breached ? 'attention' : '')}
      ${kpi('Avg end-to-end TAT', m.avgE2E == null ? '—' : fmtDur(m.avgE2E), m.recentDoneN ? `${plural(m.recentDoneN, 'approval')} in the last 30 days` : 'No approvals in the last 30 days', '', false)}
      ${kpi('On-time stages', m.onTime == null ? '—' : Math.round(m.onTime * 100) + '%', m.onTimeN ? `of ${plural(m.onTimeN, 'stage handoff')} met their SLA` : 'No completed stages yet', '', false)}
    </div>
    <div class="grid charts">
      <div class="panel" data-anim><div class="panel-head"><div><h2>Where work is sitting</h2><div class="sub">Open requests by stage · red count is over SLA</div></div></div><div class="panel-body">${m.wip.some((x) => x.count) ? chartBarsH(m.wip.map((x) => ({ label: x.stage.name, value: x.count, color: x.stage.color, extra: x.breached ? `· ${x.breached} over` : '', tip: `${x.stage.name}: ${plural(x.count, 'open request')}${x.breached ? `, ${x.breached} over SLA` : ''}` })), { aria: 'Open requests by stage' }) : emptyHtml('board', 'No open requests right now')}</div></div>
      <div class="panel" data-anim><div class="panel-head"><div><h2>Turnaround vs SLA</h2><div class="sub">Average time per stage · marker is the SLA</div></div></div><div class="panel-body">${chartBullet(m.tat)}</div></div>
    ...
    <div class="grid two">
      <div class="panel" data-anim><div class="panel-head"><div><h2>Needs attention</h2><div class="sub">Over SLA or past the due date</div></div>${attention.length ? `<span class="badge crit">${attention.length}</span>` : ''}</div><div class="panel-body" style="padding-top:6px">${attention.length ? `<div class="list">${attention.map((r) => { const s = slaState(r); const st = stageById(r.stage); const due = fmtDue(r.dueDate, r.status); return `<div class="list-item" data-open="${attr(r.id)}">${personHtml(personFor(r), 'sm')}<div class="li-main"><div class="li-title">${esc(r.id)} · ${esc(r.title)}</div><div class="li-sub">${esc(st ? st.name : '')} · ${s.state === 'crit' ? `<span style="color:var(--crit);font-weight:600">SLA breached by ${fmtDur(-s.remaining)}</span>` : `in stage ${fmtDur(s.elapsed)}`}${isOverdue(r) ? ` · <span style="color:var(--crit)">${esc(due.text)}</span>` : ''}</div></div>${stageChip(st)}</div>`; }).join('')}</div>` : emptyHtml('check', 'Everything is inside its SLA and due date')}</div></div>
...
function filteredRequests() {
  ...
  return S.requests.filter((r) => {
    if (q && !(lower(r.id).includes(q) || lower(r.title).includes(q) || lower(r.type).includes(q) || lower(r.requester && r.requester.name).includes(q) || lower(r.brief).includes(q))) return false;
...
function filtersHtml() {
  ...
  return `<div class="board-tools" data-anim>
    ...
    <select class="input sm" data-filter="type" aria-label="Filter by type" style="width:auto"><option value="">All types</option>${S.settings.types.map((t) => `<option ${f.type === t ? 'selected' : ''}>${esc(t)}</option>`).join('')}</select>
    ...
    <button class="btn sm ${f.breached ? 'primary' : ''}" data-toggle="breached">${ic('alert', 'sm')}Over SLA</button>
...
function viewBoard() {
  ...
  const head = `<div class="page-head" data-anim><div><h1>Requests</h1><p class="lede">Every requirement moving through ${stages().filter((s) => s.kind !== 'start' && s.kind !== 'end').map((s) => s.name).join(' → ')} to approval.</p></div><div class="page-actions"><button class="btn primary" data-action="new">${ic('plus')}New request</button></div></div>`;
  ...
  const cols = boardStages().map((st) => {
    ...
    return `<section class="col" style="--stage:${attr(st.color)}" data-anim><div class="col-head"><span class="swatch"></span><h3>${esc(st.name)}</h3>${st.slaHours ? `<span class="sla">SLA ${st.slaHours}h</span>` : ''}<span class="count">${items.length}${breached ? ` <span style="color:var(--crit)">· ${breached}!</span>` : ''}</span></div><div class="col-body">${items.map(cardHtml).join('') || `<div class="col-empty">${st.kind === 'end' ? 'Nothing approved yet' : 'Nothing here'}</div>`}</div></section>`;
  }).join('');
...
function viewList(rs) {
  ...
  const val = (r) => ({ id: r.num, title: lower(r.title), type: r.type, priority: S.settings.priorities.indexOf(r.priority), stage: stageIdx(r.stage), assignee: lower((personFor(r) || {}).name), inStage: slaState(r).elapsed, sla: slaState(r).ratio ?? -1, due: r.dueDate || '9999', updatedAt: r.updatedAt })[k];
  ...
  return `<div class="panel" data-anim><div class="table-wrap"><table class="tbl"><thead><tr>${th('id', 'ID')}${th('title', 'Title')}${th('type', 'Type')}${th('priority', 'Priority')}${th('stage', 'Stage')}${th('assignee', 'Assignee')}${th('inStage', 'In stage')}${th('sla', 'SLA')}${th('due', 'Due')}${th('updatedAt', 'Updated')}</tr></thead><tbody>
    ${sorted.map((r) => { const st = stageById(r.stage); const s = slaState(r); const due = fmtDue(r.dueDate, r.status); return `<tr class="link" data-open="${attr(r.id)}"><td class="num muted">${esc(r.id)}</td><td><b>${esc(r.title)}</b>${r.round > 1 ? ` <span class="round">R${r.round}</span>` : ''}${r.status === 'cancelled' ? ' <span class="chip">cancelled</span>' : ''}</td><td>${esc(r.type)}</td><td>${prioHtml(r.priority)}</td><td>${stageChip(st)}</td><td>${whoHtml(personFor(r))}</td><td class="num">${r.status === 'open' ? fmtDur(s.elapsed) : '—'}</td><td>${s.state === 'crit' ? `<span class="breach-flag">${ic('alert')}${fmtDur(-s.remaining)} over</span>` : s.state === 'warn' ? `<span class="chip warn">${fmtDur(s.remaining)} left</span>` : s.state === 'ok' ? `<span class="chip good">on track</span>` : '<span class="muted">—</span>'}</td><td class="${due.cls}">${esc(due.text)}</td><td class="muted small">${fmtRel(r.updatedAt)}</td></tr>`; }).join('') || `<tr><td colspan="10">${emptyHtml('search', 'No requests match these filters')}</td></tr>`}
  </tbody></table></div></div>`;
}
function viewQueue() {
  const mine = S.requests.filter((r) => r.status === 'open');
  const work = mine.filter((r) => needsMe(r) === 'work'), review = mine.filter((r) => needsMe(r) === 'review'), claim = mine.filter((r) => needsMe(r) === 'claim');
  const raised = S.requests.filter((r) => isRequester(r) && r.status === 'open' && needsMe(r) !== 'review');
  const sec = (title, sub, items, icon) => `<div class="panel" data-anim><div class="panel-head"><div><h2>${esc(title)}</h2><div class="sub">${esc(sub)}</div></div><span class="badge" style="${items.length ? '' : 'background:var(--surface-3);color:var(--ink-3)'}">${items.length}</span></div><div class="panel-body" style="padding-top:6px">${items.length ? `<div class="list">${items.map((r) => { const st = stageById(r.stage); const s = slaState(r); return `<div class="list-item" data-open="${attr(r.id)}"><div class="li-main"><div class="li-title">${esc(r.id)} · ${esc(r.title)}</div><div class="li-sub">${esc(r.type)} · ${esc(r.priority)} · in ${esc(st ? st.name : '')} for ${fmtDur(s.elapsed)}${s.state === 'crit' ? ` · <span style="color:var(--crit);font-weight:600">over SLA</span>` : s.state === 'warn' ? ` · <span style="color:var(--warn);font-weight:600">${fmtDur(s.remaining)} left</span>` : ''}</div></div><div class="li-side">${stageChip(st)}</div></div>`; }).join('')}</div>` : emptyHtml(icon, 'Nothing here right now')}</div></div>`;
  return `<div class="page-head" data-anim><div><h1>My queue</h1><p class="lede">What is waiting on you, ${esc(S.me.name.split(' ')[0])} — ${roles().filter((r) => hasRole(S.me, r.id)).map((r) => r.name).join(', ') || 'no role yet'}.</p></div><div class="page-actions"><button class="btn primary" data-action="new">${ic('plus')}New request</button></div></div>
  <div class="grid two">${sec('Assigned to me', 'Stages where you are the assignee', work, 'inbox')}${sec('Waiting for my review', 'Requests you raised that are ready to approve', review, 'check')}${sec('Available to claim', 'Unassigned work matching your role', claim, 'hand')}${sec('My open requests', 'Requirements you raised, still in progress', raised, 'file')}</div>`;
}
function viewTeam() {
  ...
  return `<div class="page-head" data-anim><div><h1>Team</h1><p class="lede">Who can sign in, and which stage of the relay they own. ${admin ? 'Add teammates by work email and give each a role.' : `Managed by ${esc(admins.map((a) => a.name).join(', ') || 'the admin')}.`}</p></div>
  ...
  <div class="stack">
    ...
    <div class="grid two">
      ...
      <div class="panel" data-anim><div class="panel-head"><div><h2>How access works</h2></div></div><div class="panel-body"><div class="stack" style="gap:10px;font-size:13.5px;color:var(--ink-2)">
        <p><b>1. Share the page.</b> ${S.mode === 'live' ? 'Only people this artifact is shared with can open it — that is the real access gate.' : 'In demo mode everything stays in this browser.'}</p>
        <p><b>2. Add them here.</b> A teammate signs in with the work email an admin added, and sees the actions their role allows.</p>
        <p><b>3. Roles drive the relay.</b> <b>Marketing</b> raises and approves, <b>Content</b> and <b>Creative</b> own their stages, <b>Admin</b> can change the team, the flow and settings.</p>
        <p class="muted small">Sign-in is a roster check, not a password — keep the artifact shared only with your team.</p></div></div></div>
    </div>
  </div>`;
}
function flowDiagram(flow) {
  const sts = flow.stages;
  return `<div class="flow-diagram">${sts.map((s) => { const loops = s.kind === 'review' && s.editsTo ? stageById(s.editsTo) || sts.find((x) => x.id === s.editsTo) : null; return `<div class="flow-node" style="--stage:${attr(s.color)}"><div class="fn-name"><i></i>${esc(s.name)}</div><div class="fn-meta">${s.kind === 'start' ? 'Raised by' : s.kind === 'end' ? 'Terminal' : s.kind === 'review' ? 'Reviewed by' : 'Owned by'} <b>${esc(s.kind === 'end' ? '—' : roleName(s.role))}</b></div><div class="fn-meta">${s.slaHours ? `SLA <b>${s.slaHours}h</b>` : 'No SLA'}${s.needsFile ? ' · needs file link' : ''}</div>${loops ? `<div class="fn-loop">↺ edits go back to ${esc(loops.name)}</div>` : ''}<span class="fn-arrow"></span></div>`; }).join('')}</div>`;
}
function viewFlow() {
  const admin = isAdmin();
  ...
  const editor = admin ? `<div class="panel" data-anim><div class="panel-head"><div><h2>Edit the flow</h2><div class="sub">Order, owner role, SLA hours and rules per stage. Changes apply to new handoffs; requests keep the stage they are in.</div></div><div class="row"><button class="btn sm" data-action="add-stage">${ic('plus', 'sm')}Add stage</button><button class="btn sm ghost" data-action="reset-flow">Discard changes</button><button class="btn sm primary" data-action="save-flow">${ic('check', 'sm')}Save flow</button></div></div>
    ...
    ${f.stages.map((s, i) => `<div class="stage-editor" data-i="${i}"><span class="se-idx">${i + 1}</span>
      ...
      <div class="se-field"><label>${s.kind === 'start' ? 'Raised by' : s.kind === 'review' ? 'Reviewed by' : 'Owner role'}</label><select class="input sm" data-sf="role" aria-label="Owner role" ${s.kind === 'end' ? 'disabled' : ''}>${s.kind === 'end' ? '<option value="">—</option>' : roleOpts(s.role)}</select></div>
      <div class="se-field se-sla"><label>SLA (h)</label><input class="input sm" type="number" min="0" step="1" data-sf="slaHours" value="${attr(s.slaHours || 0)}" aria-label="SLA hours" ${s.kind === 'start' || s.kind === 'end' ? 'disabled' : ''}></div>
      <div class="se-field"><label>Kind</label><select class="input sm" data-sf="kind" aria-label="Stage kind" ${i === 0 || i === f.stages.length - 1 ? 'disabled' : ''}><option value="work" ${s.kind === 'work' ? 'selected' : ''}>Work</option><option value="review" ${s.kind === 'review' ? 'selected' : ''}>Review</option>${s.kind === 'start' ? '<option value="start" selected>Start</option>' : ''}${s.kind === 'end' ? '<option value="end" selected>End</option>' : ''}</select></div>
      ${s.kind === 'review' ? `<div class="se-field"><label>Edits go to</label><select class="input sm" data-sf="editsTo" aria-label="Edits go to">${f.stages.filter((x) => x.kind === 'work').map((x) => `<option value="${attr(x.id)}" ${s.editsTo === x.id ? 'selected' : ''}>${esc(x.name)}</option>`).join('')}</select></div>` : ''}
    ...
    <div class="callout" style="margin-top:14px">The first stage is where a requirement is raised and the last is the terminal <b>Approved</b> state. A <b>Review</b> stage is decided by the person who raised the request (approve, or request edits back to the stage you choose).</div></div></div>` : '';
  ...
  <div class="stack"><div class="panel" data-anim><div class="panel-head"><div><h2>Current flow</h2><div class="sub">Each stage's owner role and SLA</div></div></div><div class="panel-body">${flowDiagram(admin ? f : S.flow)}</div></div>${editor}</div>`;
}
function viewSettings() {
  ...
  return `<div class="page-head" data-anim><div><h1>Settings</h1><p class="lede">Workspace defaults, how turnaround time is counted, and housekeeping.</p></div><div class="page-actions"><button class="btn primary" data-action="save-settings">${ic('check')}Save settings</button></div></div>
  <div class="grid two">
    <div class="stack">
      <div class="panel" data-anim><div class="panel-head"><div><h2>Turnaround clock</h2><div class="sub">How TAT and SLA hours are counted</div></div><button class="switch" role="switch" aria-checked="${w.enabled ? 'true' : 'false'}" data-action="toggle-wh" aria-label="Count working hours only"></button></div><div class="panel-body stack" style="gap:12px">
        <p class="small muted">${w.enabled ? 'Only working hours count toward a stage\'s TAT. A 24h SLA is 24 working hours.' : 'Every hour counts, including nights and weekends. Turn on to count working hours only.'}</p>
        ...
        <div class="field"><label>Working days</label><div class="days">${dayNames.map((d, i) => `<label title="${['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'][i]}"><input type="checkbox" data-day="${i}" ${w.days.includes(i) ? 'checked' : ''} ${w.enabled ? '' : 'disabled'}>${d}</label>`).join('')}</div></div>
        <div class="field"><label>Warn when a stage has used <span class="mono" id="warn-pct">${Math.round((st.slaWarnAt || 0.75) * 100)}%</span> of its SLA</label><input type="range" min="50" max="95" step="5" value="${Math.round((st.slaWarnAt || 0.75) * 100)}" data-ss="slaWarnAt" aria-label="SLA warning threshold"></div>
      </div></div>
      <div class="panel" data-anim><div class="panel-head"><div><h2>Request IDs</h2><div class="sub">Prefix for new request numbers</div></div></div><div class="panel-body"><div class="field" style="max-width:200px"><label>Prefix</label><input class="input sm mono" data-ss="prefix" value="${attr(st.prefix || 'BB')}" maxlength="6" style="text-transform:uppercase"></div></div></div>
    </div>
    <div class="stack">
      <div class="panel" data-anim><div class="panel-head"><div><h2>Request types</h2><div class="sub">What marketing can ask for</div></div></div><div class="panel-body stack" style="gap:10px"><div class="tags">${st.types.map((t, i) => `<span class="tag">${esc(t)}<button data-action="del-type" data-i="${i}" aria-label="Remove ${attr(t)}">${ic('x', 'sm')}</button></span>`).join('')}</div><form class="row" data-form="add-type"><input class="input sm" name="v" placeholder="Add a type, e.g. Reel" required><button class="btn sm">Add</button></form></div></div>
      ...
      <div class="panel" data-anim><div class="panel-head"><div><h2>Housekeeping</h2></div></div><div class="panel-body stack" style="gap:10px">
        <div class="row between wrap"><div><b class="small">Export requests</b><div class="tiny muted">CSV with every stage's TAT and SLA result</div></div><button class="btn sm" data-action="export">${ic('download', 'sm')}Download CSV</button></div>
        ${S.requests.some((r) => r.sample) || S.members.some((m) => m.sample) ? `<div class="row between wrap"><div><b class="small">Sample data</b><div class="tiny muted">${plural(S.requests.filter((r) => r.sample).length, 'sample request')} · ${plural(S.members.filter((m) => m.sample).length, 'sample teammate')}</div></div><button class="btn sm danger" data-action="remove-samples">${ic('trash', 'sm')}Remove samples</button></div>` : ''}
...
function stepperHtml(r) {
  ...
  return `<div class="stepper">${stages().map((s, i) => {
    const visits = (r.visits || []).filter((v) => v.stage === s.id); const tat = stageTat(r, s.id); const breached = visits.some(visitBreached);
    const done = r.status === 'done' ? true : i < cur || (i === cur && r.status !== 'open'); const current = r.status === 'open' && i === cur;
    const time = s.kind === 'start' ? fmtDate(r.createdAt) : s.kind === 'end' ? (r.completedAt ? fmtDate(r.completedAt) : '') : visits.length ? fmtDur(tat) + (s.slaHours ? ` / ${s.slaHours}h` : '') : '';
    return `<div class="step ${done ? 'done' : ''} ${current ? 'current' : ''}" style="--stage:${attr(s.color)}"><div class="node">${done ? ic('check', 'sm') : i + 1}</div><div class="s-name">${esc(s.name)}</div><div class="s-time ${breached ? 'crit' : ''}">${esc(time)}</div></div>`;
  }).join('')}</div>`;
}
function actionPanelHtml(r) {
  const st = stageById(r.stage); if (!st) return '';
  ...
  if (st.kind === 'end') return `<div class="action-panel done"><h3>${ic('check', 'sm')} Approved ${fmtRel(r.completedAt || r.updatedAt)}</h3><p class="small">End-to-end turnaround <b class="mono">${fmtDur(totalTat(r))}</b> across ${plural(r.round || 1, 'round')}.</p>${canEditRequest(r) ? `<div class="actions"><button class="btn sm" data-action="reopen">${ic('rotate', 'sm')}Reopen for changes</button></div>` : ''}</div>`;
  const s = slaState(r); const slaTxt = s.state === 'none' ? '' : s.state === 'crit' ? `<span class="chip crit">${ic('alert', 'sm')}SLA breached by ${fmtDur(-s.remaining)}</span>` : s.state === 'warn' ? `<span class="chip warn">${fmtDur(s.remaining)} left on SLA</span>` : `<span class="chip good">${fmtDur(s.remaining)} left on SLA</span>`;
  if (st.kind === 'review') {
    const who = r.requester ? r.requester.name : 'the requester';
    const latest = (r.deliverables || []).filter((d) => d.round === r.round);
    if (!canReview(r)) return `<div class="action-panel review"><div class="row between wrap"><h3>Waiting for ${esc(who)} to review</h3>${slaTxt}</div><p class="small muted">Round ${r.round || 1}. ${latest.length ? plural(latest.length, 'file link') + ' submitted for this round.' : 'No file link on this round yet.'}</p></div>`;
    return `<div class="action-panel review"><div class="row between wrap"><h3>Your review · round ${r.round || 1}</h3>${slaTxt}</div>
      ${latest.length ? `<div class="stack" style="gap:6px">${latest.map((d) => `<div class="deliverable">${ic('link', 'sm')}<a href="${attr(d.url)}" target="_blank" rel="noopener">${esc(d.label)}</a>${ic('external', 'sm')}</div>`).join('')}</div>` : '<p class="small muted">No file link was added on this round — ask for it in the thread or request edits.</p>'}
      <form data-form="review" class="stack" style="gap:8px"><textarea class="input" name="note" placeholder="Notes for the team — required when requesting edits" rows="3"></textarea><div class="actions"><button class="btn good" data-review="approve" type="button">${ic('check', 'sm')}Approve</button><button class="btn" data-review="edits" type="button">${ic('rotate', 'sm')}Request edits</button></div></form></div>`;
  }
  // work stage
  const a = (r.assignees || {})[st.id]; const eligible = membersWithRole(st.role); const roundFiles = (r.deliverables || []).filter((d) => d.round === r.round);
  const needFile = st.needsFile && !roundFiles.length;
  const assignUi = canAssign(r, st) ? `<div class="row wrap"><select class="input sm" data-assign-select style="width:auto;min-width:180px" aria-label="Assign ${attr(st.name)} to"><option value="">${eligible.length ? 'Choose a teammate…' : 'No one has the ' + esc(roleName(st.role)) + ' role yet'}</option>${eligible.map((m) => `<option value="${attr(m.email)}" ${a && sameEmail(a.email, m.email) ? 'selected' : ''}>${esc(m.name)}${isMe(m) ? ' (you)' : ''}</option>`).join('')}</select><button class="btn sm" data-action="assign" ${eligible.length ? '' : 'disabled'}>Assign</button>${hasRole(S.me, st.role) && !isMe(a) ? `<button class="btn sm" data-action="claim">${ic('hand', 'sm')}Claim it</button>` : ''}${a && (isAdmin() || isRequester(r)) ? `<button class="btn sm ghost" data-action="unassign">Clear</button>` : ''}</div>` : '';
  return `<div class="action-panel"><div class="row between wrap"><h3>${esc(st.name)} stage${a ? '' : ' · unassigned'}</h3>${slaTxt}</div>
    <div class="row between wrap"><span class="who">${personHtml(a)}<span class="name">${a ? esc(a.name) + (isMe(a) ? ' (you)' : '') : 'Nobody assigned yet'}</span></span>${assignUi}</div>
    ${canComplete(r, st) ? `<div class="divider" style="margin:4px 0"></div><div class="row between wrap"><span class="small muted">${needFile ? `Add the final file link below before marking ${esc(st.name)} done.` : `Done with ${esc(st.name)}? Hand it to ${esc(nextStage(st.id) ? nextStage(st.id).name : 'the next stage')}.`}</span><div class="actions">${prevStage(st.id) && prevStage(st.id).kind !== 'start' ? `<button class="btn sm ghost" data-action="sendback">${ic('arrowLeft', 'sm')}Send back</button>` : ''}<button class="btn sm primary" data-action="complete" ${needFile ? 'disabled' : ''}>${ic('arrowRight', 'sm')}Mark ${esc(st.name)} done</button></div></div>` : `<p class="small muted">${a ? `${esc(a.name)} is working on this.` : `Waiting for a ${esc(roleName(st.role))} teammate to pick it up.`}</p>`}
  </div>`;
}
function drawerHtml(r) {
  const st = stageById(r.stage); const due = fmtDue(r.dueDate, r.status);
  const visitsTl = (r.visits || []).map((v, i) => { const s = stageById(v.stage) || { name: v.stage, color: '#888' }; const el = visitElapsed(v); const sla = stageSlaMs(s); const br = sla > 0 && el > sla; const open = !v.exitedAt;
    const what = s.kind === 'start' ? `<b>${esc(r.requester ? r.requester.name : '')}</b> raised the requirement` : open ? `In <b>${esc(s.name)}</b>${v.assignee ? ` with ${esc(v.assignee.name)}` : ''} since ${fmtDateTime(v.enteredAt)}` : `<b>${esc(s.name)}</b> ${{ completed: 'completed', approved: 'approved', edits: 'edits requested', 'sent-back': 'sent back', cancelled: 'cancelled', reopened: 'reopened', submitted: 'submitted' }[v.action] || v.action || 'closed'}${v.by ? ` by ${esc(v.by.name)}` : ''}`;
    return `<div class="tl" style="--stage:${attr(s.color)}"><div class="dot ${open ? '' : 'filled'}"></div><div><div class="t-main">${what}${v.round > 1 ? ` <span class="round">R${v.round}</span>` : ''}</div><div class="t-sub"><span>${fmtDateTime(v.enteredAt)}${v.exitedAt && s.kind !== 'start' ? ' → ' + fmtDateTime(v.exitedAt) : ''}</span>${s.kind !== 'start' && s.kind !== 'end' ? `<span>TAT ${fmtDur(el)}${sla ? ` / ${s.slaHours}h` : ''}</span>${sla ? `<span class="${br ? 'crit' : 'good'}">${br ? 'SLA breached' : 'within SLA'}</span>` : ''}` : ''}</div></div></div>`; }).join('');
  return `<div class="drawer-head"><div class="grow"><div class="row wrap" style="gap:8px"><span class="mono small muted">${esc(r.id)}</span>${stageChip(st)}${r.status === 'cancelled' ? '<span class="chip">Cancelled</span>' : ''}${isBreachedNow(r) ? `<span class="breach-flag pulse">${ic('alert')}SLA breached</span>` : ''}${r.round > 1 ? `<span class="round">Round ${r.round}</span>` : ''}${r.sample ? '<span class="chip">sample</span>' : ''}</div><h2>${esc(r.title)}</h2><div class="row wrap" style="gap:10px;margin-top:8px"><span class="chip outline">${esc(r.type)}</span>${prioHtml(r.priority)}<span class="small ${due.cls}">${ic('calendar', 'sm')} ${esc(due.text)}</span><span class="who">${personHtml(r.requester, 'sm')}<span class="name small">${esc(r.requester ? r.requester.name : '')}</span></span></div></div>
  ...
  <div class="drawer-body">
    ${stepperHtml(r)}
    ${actionPanelHtml(r)}
    <section><div class="section-title">Requirement</div><div class="brief">${esc(r.brief || 'No brief provided.')}</div>${(r.refs || []).length ? `<div class="stack" style="gap:6px;margin-top:10px">${r.refs.map((u) => `<div class="deliverable">${ic('link', 'sm')}<a href="${attr(u)}" target="_blank" rel="noopener">${esc(u)}</a></div>`).join('')}</div>` : ''}</section>
    ...
    <section><div class="section-title">Thread</div><div class="thread">${(r.thread || []).length ? r.thread.map((m) => `<div class="msg ${m.kind}">${m.kind === 'system' ? '' : personHtml(m.by, 'sm')}<div class="bubble"><div class="m-head"><b>${esc(m.by ? m.by.name : 'System')}</b><span>${m.kind === 'edit' ? 'requested edits · ' : m.kind === 'approval' ? 'approved · ' : ''}${fmtRel(m.at)}</span></div><div class="m-text">${esc(m.text)}</div></div></div>`).join('') : '<p class="small muted">No messages yet.</p>'}</div>
...
function requestFormHtml(r) {
  ...
  return `<div class="form-grid">
    <div class="field span-2"><label for="f-title">What do you need?</label><input class="input" id="f-title" name="title" value="${attr(r.title || '')}" placeholder="e.g. Diwali offer — Instagram carousel" required maxlength="140"></div>
    <div class="field"><label for="f-type">Type</label><select class="input" id="f-type" name="type">${opt(S.settings.types, r.type || S.settings.types[0])}</select></div>
    ...
    <div class="field span-2"><label for="f-brief">Brief</label><textarea class="input" id="f-brief" name="brief" rows="5" placeholder="Audience, message, format and sizes, tone, must-haves, deadline context…" required>${esc(r.brief || '')}</textarea><span class="hint">The content and creative teams work from this — the more specific, the fewer edit rounds.</span></div>
    <div class="field span-2"><label for="f-refs">Reference links</label><textarea class="input" id="f-refs" name="refs" rows="2" placeholder="One link per line — past creatives, brand folder, competitor examples">${esc((r.refs || []).join('\n'))}</textarea></div>
  </div>`;
}
function modalHtml(m) {
  ...
  switch (m.type) {
    case 'new': return `<form data-form="new">${wrap('New request', `Goes straight to <b>${esc(firstWorkStage().name)}</b> for pickup.`, requestFormHtml(), `<button class="btn" type="button" data-action="close-modal">Cancel</button><button class="btn primary" type="submit">${ic('send', 'sm')}Raise request</button>`)}</form>`;
...
function gateHtml() {
  ...
  const body = S.bootstrap ? `<form data-form="bootstrap" class="stack" style="gap:12px"><div class="field"><label for="g-name">Your name</label><input class="input" id="g-name" name="name" required placeholder="Pawan Kumar" value="${attr(APP.ownerName)}"></div><div class="field"><label for="g-email">Work email</label><input class="input" id="g-email" name="email" type="email" required value="${attr(APP.ownerEmail)}"></div><button class="btn primary block" type="submit">Set up workspace as admin</button></form>`
    : `<form data-form="signin" class="stack" style="gap:12px"><div class="field"><label for="g-email">Work email</label><input class="input" id="g-email" name="email" type="email" required placeholder="name@bricknbolt.com" autocomplete="email" ${loading ? 'disabled' : ''}>${S.gateError ? `<span class="error">${esc(S.gateError)}</span>` : ''}</div><button class="btn primary block" type="submit" ${loading ? 'disabled' : ''}>${loading ? 'Loading team…' : 'Continue'}</button></form>`;
  return `<div class="gate"><div class="gate-card" data-anim><div class="row"><div class="brand-mark">${ic('relay')}</div><div><div class="brand-name">Relay</div><div class="brand-sub">Creative ops · Brick&amp;Bolt</div></div></div>
    <div><h1>${S.bootstrap ? 'Set up your workspace' : 'Who\'s working?'}</h1><p class="lede" style="margin-top:6px">${S.bootstrap ? 'No team yet — the first person in becomes the admin.' : 'Sign in with the work email your admin added to the team.'}</p></div>
    ${body}
    <div class="gate-foot">${pill}<br><span style="display:inline-block;margin-top:8px">${admins.length ? `Not on the team? Ask ${esc(admins.join(' or '))}.` : ''}</span></div></div></div>`;
}
function trySignIn(email) {
  ...
  if (sameEmail(email, APP.ownerEmail)) { const doc = { email, name: APP.ownerName, roles: ['admin', 'requester'], active: true }; saveMember(doc, true).then(() => { localStorage.setItem(LS.me, email); S.me = S.members.find((x) => sameEmail(x.email, email)); renderAll(true); }); return; }
  S.gateError = 'That email is not on the team yet. Ask an admin to add you.'; renderAll(false);
}
...
function navItems(extraCls = '') {
  const m = metrics();
  return NAV.filter((n) => !n.admin || isAdmin()).map((n) => { const badge = n.v === 'queue' && m.mine ? `<span class="badge">${m.mine}</span>` : n.v === 'board' && m.breached ? `<span class="badge crit" title="SLA breached">${m.breached}</span>` : ''; return `<a class="nav-item ${extraCls} ${S.route.view === n.v ? 'active' : ''}" href="#/${n.v}" data-nav="${n.v}" aria-current="${S.route.view === n.v ? 'page' : 'false'}">${ic(n.icon)}<span>${esc(n.label)}</span>${badge}</a>`; }).join('');
}
function shellHtml() {
  ...
  return `<div class="app" id="app">
    ...
    <div class="main"><header class="topbar"><span class="title">${esc(title)}</span><div class="search">${ic('search')}<input type="search" id="q" placeholder="Search requests, IDs, people…" value="${attr(S.q)}" aria-label="Search"></div><span class="grow"></span>${pill}<button class="btn primary sm" data-action="new">${ic('plus', 'sm')}<span class="nowrap">New request</span></button></header>
...
function renderAll(animate) {
  ...
  else { $$('.nav', root).forEach((n) => { const items = n.classList.contains('bottom-nav') ? navItems() : navItems(); const holder = n.classList.contains('bottom-nav') ? n : null; if (holder) holder.innerHTML = items; else { const first = n.querySelector('.nav-item'); const foot = n.querySelector('.nav-spacer'); $$('.nav-item[data-nav]', n).forEach((x) => x.remove()); foot.insertAdjacentHTML('beforebegin', items); } }); $('.topbar .title').textContent = (NAV.find((n) => n.v === S.route.view) || {}).label || ''; }
  renderPage(animate);
  syncLayer();
}
...
function syncLayer() {
  ...
  else { $$('.closing', layer).forEach((el) => el.remove()); layer.innerHTML = `<div class="overlay" data-action="close-drawer"></div><aside class="drawer" role="dialog" aria-label="${attr(r.id)}">${drawerHtml(r)}</aside>`; }
...
function captureForms(root) { const v = {}; $$('textarea, input:not([type=checkbox]):not([type=radio]), select', root).forEach((el, i) => { const k = (el.name || el.id || '') + '#' + i; v[k] = el.value; }); v.__active = document.activeElement && root.contains(document.activeElement) ? ((document.activeElement.name || document.activeElement.id || '') + '#' + $$('textarea, input:not([type=checkbox]):not([type=radio]), select', root).indexOf(document.activeElement)) : null; return v; }
function restoreForms(root, vals) { if (!vals) return; $$('textarea, input:not([type=checkbox]):not([type=radio]), select', root).forEach((el, i) => { const k = (el.name || el.id || '') + '#' + i; if (k in vals && vals[k] !== '' && el.value !== vals[k]) el.value = vals[k]; if (vals.__active === k) { el.focus(); try { el.setSelectionRange(el.value.length, el.value.length); } catch (e) { /* not all inputs */ } } }); }
...
function toast(msg, kind = 'good') {
  ...
  setTimeout(() => { if (motionOk()) gsap.to(el, { opacity: 0, y: 8, duration: 0.25, onComplete: () => el.remove() }); else el.remove(); }, kind === 'crit' ? 5200 : 3200);
}
const tip = { el: null };
...
function csvExport() {
  ...
  const head = ['ID', 'Title', 'Type', 'Priority', 'Status', 'Stage', 'Round', 'Requester', 'Raised', 'Due', 'Approved', 'Total TAT (h)', ...sts.flatMap((s) => [`${s.name} TAT (h)`, `${s.name} SLA (h)`, `${s.name} result`])];
  const q = (v) => `"${String(v ?? '').replace(/"/g, '""')}"`;
  const rows = S.requests.map((r) => [r.id, r.title, r.type, r.priority, r.status, (stageById(r.stage) || {}).name, r.round || 1, r.requester ? r.requester.name : '', new Date(r.createdAt).toISOString(), r.dueDate || '', r.completedAt ? new Date(r.completedAt).toISOString() : '', (totalTat(r) / H).toFixed(1), ...sts.flatMap((s) => { const vs = (r.visits || []).filter((v) => v.stage === s.id); if (!vs.length) return ['', s.slaHours || '', '']; const tat = stageTat(r, s.id); return [(tat / H).toFixed(1), s.slaHours || '', vs.some(visitBreached) ? 'breached' : s.slaHours ? 'within SLA' : 'no SLA']; })]);
  return [head, ...rows].map((r) => r.map(q).join(',')).join('\n');
}
async function doExport() {
  const data = csvExport(); const filename = `relay-requests-${isoToday()}.csv`;
...
function parseRequestForm(form) {
  ...
  return { title: f.title.trim(), brief: f.brief.trim(), type: f.type, priority: f.priority, dueDate: f.dueDate || null, refs };
}
...
document.addEventListener('click', async (e) => {
  ...
  const sortEl = e.target.closest('[data-sort]'); if (sortEl) { const k = sortEl.dataset.sort; if (S.sort.key === k) S.sort.dir = S.sort.dir === 'asc' ? 'desc' : 'asc'; else S.sort = { key: k, dir: k === 'title' || k === 'assignee' || k === 'type' ? 'asc' : 'desc' }; renderPage(false); return; }
  const revEl = e.target.closest('[data-review]'); if (revEl) { const form = revEl.closest('form'); const note = form.querySelector('[name=note]').value.trim(); const id = S.drawer; if (revEl.dataset.review === 'approve') { if (!(await confirmBox({ title: 'Approve this request?', sub: 'It moves to Approved and the total turnaround is recorded.', confirm: 'Approve' }))) return; await actApprove(id, note); } else { if (!note) { toast('Tell the team what to change before requesting edits.', 'crit'); form.querySelector('[name=note]').focus(); return; } await actRequestEdits(id, note); } return; }
  ...
  switch (a) {
    ...
    case 'signout': localStorage.removeItem(LS.me); S.me = null; S.drawer = null; S.modal = null; S.flowDraft = null; S.settingsDraft = null; renderAll(true); break;
    case 'edit': openModal({ type: 'edit', id }); break;
    case 'assign': { const sel = $('[data-assign-select]'); if (!sel || !sel.value) return toast('Choose a teammate first.', 'info'); await actAssign(id, getReq(id).stage, sel.value); break; }
    case 'claim': await actAssign(id, getReq(id).stage, S.me.email); break;
    case 'unassign': await actAssign(id, getReq(id).stage, null); break;
    case 'complete': await actComplete(id); break;
    case 'sendback': { const st = stageById(getReq(id).stage); const pv = prevStage(st.id); const note = await noteBox({ kind: 'sendback', title: `Send back to ${pv.name}?`, sub: 'Say what is missing so the earlier stage can fix it.', label: 'Reason', required: true, confirm: 'Send back', placeholder: 'e.g. Brief is missing the sizes we need' }); if (note === false) return; await actSendBack(id, note); break; }
    case 'reopen': if (await confirmBox({ title: 'Reopen this request?', sub: 'It goes back into the flow as a new round.', confirm: 'Reopen' })) await actReopen(id); break;
    ...
    case 'remove-member': { const m = S.members.find((x) => x.id === id); if (await confirmBox({ title: `Remove ${m ? m.name : 'this teammate'}?`, sub: 'They can no longer sign in. Their past work stays on the requests.', confirm: 'Remove', danger: true })) await removeMember(id); break; }
    ...
    case 'add-stage': { const f = S.flowDraft; const n = f.stages.length; const name = 'New stage'; f.stages.splice(n - 1, 0, { id: slug(name) + '-' + uid().slice(0, 4), name, role: roles().find((r) => !['admin', 'requester'].includes(r.id))?.id || 'content', slaHours: 24, kind: 'work', color: '#0E9384', needsFile: false }); renderPage(false); break; }
    ...
    case 'stage-del': { const f = S.flowDraft; const s = f.stages[Number(act.dataset.i)]; if (await confirmBox({ title: `Remove the ${s.name} stage?`, sub: 'Past turnaround data for this stage stays on old requests.', confirm: 'Remove', danger: true })) { f.stages.splice(Number(act.dataset.i), 1); f.stages.forEach((x) => { if (x.editsTo === s.id) x.editsTo = f.stages.find((y) => y.kind === 'work')?.id || ''; }); renderPage(false); } break; }
    ...
    case 'save-settings': { const st = clone(S.settingsDraft); st.prefix = (st.prefix || 'BB').toUpperCase().replace(/[^A-Z0-9]/g, '').slice(0, 6) || 'BB'; if (!st.types.length) return toast('Keep at least one request type.', 'crit'); if (!st.priorities.length) return toast('Keep at least one priority.', 'crit'); if (st.workHours.enabled && st.workHours.end <= st.workHours.start) return toast('The working day must end after it starts.', 'crit'); if (st.workHours.enabled && !st.workHours.days.length) return toast('Pick at least one working day.', 'crit'); await saveSettings(st); S.settingsDraft = clone(S.settings); renderPage(false); break; }
    case 'export': await doExport(); break;
    case 'remove-samples': if (await confirmBox({ title: 'Remove all sample data?', sub: 'Sample requests and sample teammates are deleted for everyone.', confirm: 'Remove samples', danger: true })) await removeSamples(); break;
...
function validateFlow(f) {
  if (f.stages.length < 3) return 'A flow needs at least a start, one work stage and an end.';
...
document.addEventListener('submit', async (e) => {
  ...
  switch (kind) {
    ...
    case 'new': { const d = parseRequestForm(form); if (!d) return; const btn = form.querySelector('[type=submit]'); btn.disabled = true; closeModal(); const r = await createRequest(d); openRequest(r.id); break; }
    case 'edit': { const d = parseRequestForm(form); if (!d) return; closeModal(); await actEdit(form.dataset.id, d); break; }
    case 'member': { const rolesSel = [].concat(f.roles || []); if (!rolesSel.length) return toast('Pick at least one role.', 'crit'); const existing = form.dataset.id ? S.members.find((x) => x.id === form.dataset.id) : null; if (!existing && S.members.some((x) => sameEmail(x.email, f.email))) return toast('That email is already on the team.', 'crit'); const wasAdminSelf = existing && isMe(existing) && hasRole(existing, 'admin'); if (wasAdminSelf && !rolesSel.includes('admin') && !sameEmail(existing.email, APP.ownerEmail)) return toast('You cannot remove your own admin role.', 'crit'); closeModal(); await saveMember({ ...(existing || {}), email: existing ? existing.email : f.email, name: f.name, roles: rolesSel, active: existing ? !!f.active : true }, !existing); if (existing && isMe(existing)) S.me = S.members.find((x) => x.id === existing.id) || S.me; renderAll(false); break; }
    ...
    case 'add-type': { const v = f.v.trim(); if (v && !S.settingsDraft.types.includes(v)) S.settingsDraft.types.push(v); renderPage(false); break; }
...
document.addEventListener('input', (e) => {
  ...
  if (t.dataset.sf !== undefined && S.flowDraft) { const row = t.closest('.stage-editor'); const s = S.flowDraft.stages[Number(row.dataset.i)]; const k = t.dataset.sf; if (k === 'needsFile') s.needsFile = t.checked; else if (k === 'slaHours') s.slaHours = Math.max(0, Number(t.value) || 0); else s[k] = t.value; if (k === 'kind') { if (s.kind === 'review' && !s.editsTo) s.editsTo = S.flowDraft.stages.find((x) => x.kind === 'work')?.id || ''; renderPage(false); } if (k === 'color' || k === 'name' || k === 'slaHours' || k === 'role') { const fd = $('.flow-diagram'); if (fd) fd.outerHTML = flowDiagram(S.flowDraft); } return; }
...
function subscribeAll() {
  ...
  S.store.subscribe('config', (rows) => { const flow = rows.find((r) => r.id === 'flow'); const settings = rows.find((r) => r.id === 'settings'); if (flow && Array.isArray(flow.stages) && flow.stages.length) S.flow = flow; if (settings) S.settings = { ...clone(DEFAULT_SETTINGS), ...settings, workHours: { ...DEFAULT_SETTINGS.workHours, ...(settings.workHours || {}) } }; S.loaded.config = true; renderAll(false); }, onErr);
}
async function boot() {
  ...
  else { S.store = new LocalStore(); S.mode = 'demo'; const cfg = await S.store.get('config', 'flow'); if (!cfg) { /* defaults in memory */ } }
  subscribeAll();
